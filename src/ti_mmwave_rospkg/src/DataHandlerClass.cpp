...
DataUARTHandlerNode::DataUARTHandlerNode() : Node("DataUARTHandlerNode")
{
    ...
    this->declare_parameter("root_topic",rclcpp::PARAMETER_STRING); 
...
void DataUARTHandler::init_publishers(std::shared_ptr<rclcpp::Node> nh) {

    radar_scan_pcl_pub = nh->create_publisher<sensor_msgs::msg::PointCloud2>(std::string(rootTopic) + "/radar_scan_pcl", 100);
    radar_track_marker_pub = nh->create_publisher<visualization_msgs::msg::Marker>(std::string(rootTopic) + "/radar_track_marker", 100);

    radar_scan_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarScan>(std::string(rootTopic) + "/radar_scan", 100);
    radar_occupancy_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarOccupancy>(std::string(rootTopic) + "/radar_occupancy", 100);
    radar_trackid_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarPointTrackID>(std::string(rootTopic) + "/radar_point_track_id", 100);
    radar_trackarray_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarTrackArray>(std::string(rootTopic) + "/radar_track_array", 100);
    
    radar_mdoppler_data_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarMicroDopplerDataArray>(std::string(rootTopic) + "/radar_micro_doppler_data_array", 100);
    radar_mdoppler_feature_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarMicroDopplerFeatureArray>(std::string(rootTopic) + "/radar_micro_doppler_feature_array", 100);
    radar_classifier_pub = nh->create_publisher<ti_mmwave_rospkg_msgs::msg::RadarClassifier>(std::string(rootTopic) + "/radar_classifier", 100);
...
void DataUARTHandler::setRootTopic(char const* myRootTopic)
{
	rootTopic = myRootTopic;
}
...
int main(int argc, char **argv)
{
    ...
    std::string myRootTopic = node->get_parameter("root_topic").as_string();
    ...
    d.setRootTopic(myRootTopic.c_str());
    d.init_publishers(node);   
    d.start();
